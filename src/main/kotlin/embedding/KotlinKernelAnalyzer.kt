...
import stochastic.GradientDescenter
...
data class TopicMixtureResult(val results: SortedMap<String, Double>, val kld: Double) {
    ...
    fun reportResults(doLog: Boolean = false) {
        results.entries
            .filter { (_, weight) -> weight > 0.0 }
            .sortedByDescending { (_, weight) -> weight }
            .forEach { (topicName, weight) ->
                if (doLog)
                    println("$topicName : ${log(weight)}")
                else
    ...
    fun fitDecay() {
        val nonzero = results.entries
            .filter { (_, weight) -> weight > 0.0 }
            .sortedByDescending { it.value }
        val res = nonzero.windowed(2).map { (f1, f2) ->
            f2.value/f1.value
        }.average()

        val intercept = 1.0
        nonzero.mapIndexed { index, mutableEntry ->
//            mutableEntry.setValue(intercept * pow(res, index.toDouble() + 0.0))
//            mutableEntry.setValue(mutableEntry.value * pow(res, index.toDouble() + 1.0))
//            mutableEntry.setValue(mutableEntry.value * exp(-0.12 * index.toDouble()))
        }
        val total = nonzero.sumByDouble { it.value }
        nonzero.forEach { it.setValue(it.value / total) }
    ...
    fun fitLog() {
        ...
        val total = nonzero.windowed(2).map { (f1, f2) -> abs(ln(f1.value).absoluteValue - ln(f2.value).absoluteValue)  }
            .average()
//        val slope = nonzero.sumByDouble { ln(it.value) - total } / nonzero.size

        val intercept = ln(nonzero[0].value)
//        nonzero.mapIndexed { index, mutableEntry ->
//            mutableEntry.setValue(exp(intercept - total * index))
//        }
        println("Intercept: $intercept, Slope: $total")
        nonzero.mapIndexed { index, mutableEntry ->
            mutableEntry.setValue(abs(intercept) * abs(total * index))
            println(mutableEntry.value)
        }
        val newTotal = nonzero.sumByDouble { it.value }
        nonzero.forEach { v -> v.setValue(v.value / newTotal) }
    }
...
class KernelDist(val mean: Double, val std: Double, val doCondition: Boolean = true,
    ...
    // Used for embedding
    fun perturb(nSamples: Int = 50, pmean: Double = 1.0, psd: Double = 0.001): Pair<List<String>, List<List<Double>>> {
        val norm = NormalDistribution(sharedRand, pmean, psd)
...
//        val out = ObjectOutputStream(File("Weee.txt").outputStream())
//        val wee = ObjectInputStream(File("Weee.txt").inputStream())
//        val huh = wee.readObject() as List<List<Double>>
//        huh.forEachIndexed{ i, lst ->
//            lst.forEachIndexed{ j, v -> if (v != featureFreqs[i][j]) println("BAD: ${featureNames[i]}: ${v - featureFreqs[i][j]}")}
//         }
//        out.writeObject(featureFreqs)
